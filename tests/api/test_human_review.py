...
def test_pending_review_returns_only_flagged_runs(client):
    flagged = _make_stored_run(needs_review=True)
    _make_stored_run(needs_review=False)  # should NOT appear
    r = client.get("/api/v1/gate/pending-review")
...
def test_pending_review_excludes_already_overridden(client):
    _make_stored_run(needs_review=True, override_verdict="kill")
    r = client.get("/api/v1/gate/pending-review")
    assert r.json()["pending_count"] == 0


def test_human_override_records_decision(client):
...
def test_human_override_rejects_run_not_flagged(client):
...
def test_human_override_rejects_double_override(client):
...
def test_human_override_rejects_invalid_verdict(client):
...
def test_human_override_rejects_unknown_run_id(client):
    r = client.post(
...
def test_human_override_rejects_invalid_uuid_format(client):
    r = client.post(
...
def test_human_override_rejects_short_reason(client):
    run_id = _make_stored_run(needs_review=True)
    r = client.post(
        f"/api/v1/gate/runs/{run_id}/human-override",
        json={
